...
namespace RaceGame
{
    ...
    {
        public static void DrawArrow(Vector3 pos, Vector3 direction, float arrowLength, float arrowHeadLength = 0.25f,
            float arrowHeadAngle = 20.0f)
        {
            ...
            if (c == null) return;
            Vector3 right = Quaternion.LookRotation(direction, c.transform.forward) *
                            Quaternion.Euler(0, 180 + arrowHeadAngle, 0) * new Vector3(0, 0, 1);
            Vector3 left = Quaternion.LookRotation(direction, c.transform.forward) *
                           Quaternion.Euler(0, 180 - arrowHeadAngle, 0) * new Vector3(0, 0, 1);
